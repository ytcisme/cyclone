...
const (
	// RepoRead represents reading permission in repo.
	RepoRead = "RepoRead"
	// RepoWrite represents writing permission in repo.
	RepoWrite = "RepoWrite"
...
func (server *AuthorizationsService) CreateAccessToken(ctx context.Context, user string, permissionReq PermissionReq) (string, *http.Response, error) {
	...
	req, err := server.v1Client.NewRequest("PUT", u, permissionReq, nil)
	if err != nil {
		return "", nil, err
	}
