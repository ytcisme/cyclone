...
const SupportAccessTokenVersion = 5.5
...
func (b *BitbucketServer) GetToken() (string, error) {
	if b.scmCfg.AuthType == v1alpha1.AuthTypePassword {
		...
		if version > SupportAccessTokenVersion {
			opt := PermissionReq{
				Name:        "continuous-integration/cyclone",
				Permissions: []string{ProjectRead, RepoRead},
			}
			token, _, err := b.v1Client.Authorizations.CreateAccessToken(context.Background(), b.scmCfg.User, opt)
			if err != nil {
				return "", err
			}
			return fmt.Sprintf("%s:%s", b.scmCfg.User, token), nil
		}
		return fmt.Sprintf("%s:%s", b.scmCfg.User, b.scmCfg.Password), nil
	}

	return fmt.Sprintf("%s:%s", b.scmCfg.User, b.scmCfg.Token), nil
}
	...
	for {
		...
		if err != nil {
			if resp != nil && resp.StatusCode == 500 {
				return nil, cerr.ErrorSCMServerInternalError.Error(b.scmCfg.Server, err)
			}
			return nil, err
		}
...
func (b *BitbucketServer) CreateWebhook(repo string, webhook *scm.Webhook) error {
	...
	if err != nil && !cerr.ErrorContentNotFound.Derived(err){
			return err
	}
...
func GetBitbucketVersion(client *http.Client, base *url.URL) (float32, error) {
	...
	return 0, fmt.Errorf("status: %v, Body: %s", resp.Status, string(bodyBytes))
}
